...
<figure class="half">
	...
	<figcaption>Figure 1: Reverse KL vs Forward KL divergence: The left image has a better approximation Q(x) on part of $\mathrm{P}(\mathrm{x})$. (a) Reverse KL simulation (b) Forward KL simulation</figcaption>
</figure>
...
=& \frac{N}{2} \log \tau-\frac{\tau}{2} \sum_{i=1}^{N}\left(x_{i}-\mu\right)^{2}+\frac{1}{2} \log \left(\kappa_{0} \tau\right)-\frac{\kappa_{0} \tau}{2}\left(\mu-\mu_{0}\right)^{2} \\
&+\left(a_{0}-1\right) \log \tau-b_{0} \tau+C
\end{aligned}
$$
